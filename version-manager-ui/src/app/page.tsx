...
export default function UploadPage() {
  ...
  const fetchBuilds = async (jwt?: string | null) => {
    try {
        const res = await fetch('https://maxshapez.ddns.net:3001/builds', {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoginError('');
    try {
      const res = await fetch('https://maxshapez.ddns.net:3001/login', {
        method: 'POST',
  ...
  const deleteBuild = async (id: number) => {
    ...
    try {
      const res = await fetch(`https://maxshapez.ddns.net:3001/builds/${id}`, {
        method: 'DELETE',
